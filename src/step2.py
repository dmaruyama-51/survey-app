...
def split_numeric_and_non_numeric_columns(
    ...
    try:
        st.markdown("<div class='tight-header'><h4>Column Selection</h4></div>", unsafe_allow_html=True)
        st.write("Select columns that should not be processed as numeric data (e.g., ID, category columns)")
...
def select_likert_scale_points() -> int | None:
    """リッカート尺度のポイント数を選択する"""
    try:
        st.markdown("<div class='tight-header'><h4>Likert Scale Configuration</h4></div>", unsafe_allow_html=True)
        st.write("Select the number of points used in your Likert scale data. (Sample data uses a 7-point scale)")
...
def check_step2_completion(remove_cols: list[str], likert_scale: int | None) -> bool:
    ...
    if not remove_cols and likert_scale is None:
        st.info("Please start by selecting columns to exclude and configuring the Likert scale.", icon="ℹ️")
        return False
