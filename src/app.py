...
from src.step2 import split_numeric_and_non_numeric_columns, select_likert_scale_points, check_step2_completion
...
def main():
    try:
        ...
        # -----------------------------------
        st.markdown("<div class='tight-header step1-header'><h3>📌 Step 1: Upload Survey Data</h3></div><hr/>", unsafe_allow_html=True)
        try:
            df = load_survey_data()
            if df is not None:
        ...
        # -----------------------------------
        st.markdown("<div class='tight-header step-header'><h3>📌 Step 2: Configure Data Settings</h3></div><hr/>", unsafe_allow_html=True)
        try:
            ...
            if step2_completed:
                st.markdown("<div class='tight-header step-header'><h3>📌 Step 3: Clean Data and Download Results</h3></div><hr/>", unsafe_allow_html=True)
                try:
