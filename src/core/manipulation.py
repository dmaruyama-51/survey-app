...
def reverse_score(df: pd.DataFrame, columns: List[str], scale_points: int) -> pd.DataFrame:
    """
    選択された列の得点を逆転させる
...
def prepare_download_data(df: pd.DataFrame, reversed_df: pd.DataFrame, 
                        reverse_columns: List[str], include_original: bool) -> pd.DataFrame:
    """ダウンロード用のデータフレームを準備"""
